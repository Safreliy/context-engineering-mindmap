      ...
      children: [
        ...
        },
        {
          ...
          viz: {
            type: "tool-pruning",
            caption:
              "Механика проста: большой результат инструмента перестаёт жить в активном окне целиком и заменяется коротким следом, который можно повторно раскрыть при необходимости.",
            beforeTitle: "До pruning",
            beforeLines: [
              "read_file -> 5000 строк",
              "pytest -> длинный traceback",
              "search -> 40 совпадений"
            ],
            afterTitle: "После pruning",
            afterLines: [
              "read_file -> ключевой фрагмент + ссылка",
              "pytest -> exit code + 2 ошибки",
              "search -> 3 лучших совпадения"
            ],
            note:
              "Идея не в том, чтобы потерять вывод, а в том, чтобы убрать его из горячего окна и вернуть только при новом запросе."
          },
      ...
      children: [
        {
          ...
          viz: {
            type: "compaction-flow",
            caption:
              "Старый хвост беседы превращается не в литературный пересказ, а в компактное рабочее состояние, с которым можно продолжить следующий шаг.",
            before: {
              title: "До",
              text: "Сообщения, логи, старые tool results",
              tone: "warning"
            },
            transform: {
              title: "Compaction",
              text: "Вытащить цель, ограничения, решения, незакрытые вопросы",
              tone: "soft"
            },
            after: {
              title: "После",
              text: "Краткое состояние + свежий контекст + ссылки на артефакты",
              tone: "accent"
            },
            beforeMeterLabel: "Переполненный хвост",
            beforeMeterPercent: 91,
            afterMeterLabel: "После сжатия",
            afterMeterPercent: 42,
            stages: ["Рост окна", "Порог", "Compaction", "Продолжение"],
            startMeterPercent: 42,
            peakMeterPercent: 91,
            settledMeterPercent: 43,
            thresholdPercent: 80,
            windowTitle: "Активное окно",
            windowLines: [
              {
                text: "Последние сообщения по задаче",
                old: false
              },
              {
                text: "Свежий вывод тестов и поиска",
                old: false
              },
              {
                text: "Старый хвост обсуждения",
                old: true
              },
              {
                text: "Старые tool results и логи",
                old: true
              }
            ],
            summaryTitle: "Компактное состояние",
            summaryLines: [
              "цель и текущий план",
              "критичные файлы и ограничения",
              "что уже проверили и исключили",
              "ссылки на артефакты вместо полного хвоста"
            ],
            reinjectedLine: "Компактное состояние прошлого цикла",
            statuses: {
              grow: "Контекст снова растёт: в активное окно добавляются новые сообщения и результаты инструментов.",
              threshold: "Окно подошло к критическому порогу: дальше продолжать без compaction рискованно.",
              compact: "Запускается compaction: старый хвост сворачивается в рабочее состояние и ссылки на артефакты.",
              continue: "Агент продолжает работу уже с компактным окном и запасом места под следующий цикл."
            }
          },
          sourceIds: ["anthropic-context", "anthropic-context-management"]
        },
        {
          ...
          viz: {
            type: "summary-pyramid",
            title: "Как строится иерархическая сводка",
            caption:
              "Идея не в том, чтобы один раз грубо сжать весь хвост, а в том, чтобы собирать несколько уровней сводок: локальные, промежуточные и верхний статус.",
            stages: ["Локальные заметки", "Промежуточный слой", "Верхняя сводка", "Возврат в работу"],
            stageDurations: [2600, 3200, 3200, 3400],
            statuses: {
              local:
                "Каждый подэтап получает собственную маленькую заметку с деталями, которые важны именно для этого участка работы.",
              middle:
                "Дальше несколько локальных заметок сворачиваются в промежуточные блоки по темам или веткам исследования.",
              top:
                "На верхнем уровне остаётся только краткий рабочий статус, который помогает продолжать сессию без лишнего шума.",
              reenter:
                "При продолжении работы агент может спуститься с верхнего уровня к нужной промежуточной или локальной заметке, а не разжимать всю историю целиком."
            },
            levels: [
              {
                ...
                tone: "soft",
                lines: [
                  "Подэтап A: где падает тест и какие файлы уже проверены",
                  "Подэтап B: какие гипотезы по cache invalidation уже отсеяны",
                  "Подэтап C: что показал отдельный сабагент по rename-flow"
                ]
              },
              {
                title: "Промежуточные сводки",
                tone: "accent",
                lines: [
                  "Ветка тестов: воспроизведение подтверждено, flaky-путь исключён",
                  "Ветка кода: проблема локализована вокруг stale key после rename"
                ]
              },
              {
                title: "Верхняя сводка",
                tone: "warning",
                lines: [
                  "Цель: исправить stale key после rename",
                  "Статус: источник найден, следующий шаг — правка invalidation"
                ]
              }
            ],
            returnTitle: "Возврат к деталям",
            returnPending:
              "Пока работа идёт на верхнем уровне, агент не тащит в окно все локальные заметки сразу.",
            returnLine:
        ...
        },
        {
          ...
          viz: {
            type: "summary-check",
            title: "Как проверять summary",
            caption:
              "После сжатия важно не просто принять красивую сводку, а быстро проверить, не выпали ли рабочие ограничения, путь к проблеме и незакрытые блокеры.",
            stages: ["Черновик summary", "Проверка", "Обнаружен пропуск", "Исправленный summary"],
            stageDurations: [2600, 3000, 3200, 3400],
            statuses: {
              draft:
                "Сначала появляется черновая summary: она уже короче истории, но ещё не гарантирует, что в ней остались все рабочие опорные точки.",
              review:
                "Дальше запускается короткий checklist: цель, последний падающий тест, ключевые ограничения, принятые решения и открытые блокеры.",
              gap:
                "Проверка находит конкретный пропуск. Это важнее общего впечатления, что summary выглядит связной и аккуратной.",
              fixed:
                "После правки summary снова годится для reinjection: в ней есть не только рассказ, но и рабочие координаты для следующего шага."
            },
            summaryTitle: "Рабочая summary",
            summaryLines: [
              "Цель: исправить падение после rename",
              "Найден источник: stale key в cache invalidation",
              "Решение: обновить invalidation и проверить rename-flow"
            ],
            checklistTitle: "Checklist проверки",
            checklist: [
              {
                label: "Текущая цель",
                passAt: 1
            ...
            missingLine:
              "Пропуск: не указан конкретный падающий тест и незакрытый blocker по rename-flow.",
            fixedLine:
              "Исправлено: добавлены `rename_cache_spec` и blocker по повторной проверке после правки."
          },
      ...
      children: [
        {
          ...
          viz: {
            type: "context-stack",
            title: "Из чего собирается harness",
            caption:
              "Harness удобно понимать как стек слоёв, которые вместе управляют поведением агента: от правил до инструментов и цикла выполнения.",
            layers: [
              {
                title: "System rules",
                text: "Базовые ограничения и поведение модели",
                tone: "accent"
              },
              {
                title: "Project rules",
                text: "Файлы вроде AGENTS.md или CLAUDE.md",
                tone: "soft"
              },
              {
                title: "Context policy",
                text: "Что читать, что держать, когда компактить",
                tone: "default"
              },
              {
                title: "Tools and MCP",
                text: "Какие внешние возможности доступны и как они описаны",
                tone: "soft"
              },
              {
                title: "Execution loop",
                text: "Когда запускать tool loop, проверки, handoff и завершение",
                tone: "warning"
              }
            ],
            note:
              "Разные продукты называют это по-разному, но архитектурно вопрос один: кто управляет жизненным циклом контекста."
          },
        ...
        },
        {
          ...
          viz: {
            ...
            title: "Как работает handoff",
            caption:
              "Сабагент нужен не для красоты, а чтобы вынести побочную траекторию в отдельное окно и вернуть назад только полезный результат.",
            stages: ["Делегирование", "Изолированное окно", "Артефакт", "Возврат"],
            stageDurations: [2800, 3600, 3400, 3600],
            statuses: {
              delegate:
                "Координатор формулирует подзадачу, но не раздувает главное окно всем будущим следом исследования.",
              isolate:
                "Сабагент читает файлы, запускает проверки и накапливает длинную рабочую траекторию в своём отдельном окне.",
              artifact:
                "Результат собирается в адресуемый артефакт: туда уходит длинная история, спорные места и список затронутых файлов.",
              return:
                "Назад в основное окно возвращаются только краткий вывод, путь к артефакту и несколько опорных фактов для следующего шага."
            },
            from: {
              title: "Координатор",
              text: "Формулирует подзадачу и границы",
              tone: "accent"
            },
            worker: {
              title: "Сабагент",
              text: "Исследует или правит в отдельном окне",
              tone: "soft"
            },
            artifact: {
              title: "Артефакт",
              text: "notes.md, diff, список файлов, вывод проверки",
              tone: "warning"
            },
            keep: {
              title: "Главное окно",
              text: "Не тащит весь след поиска",
              tone: "default"
            },
            returned: {
              title: "Возврат",
              text: "Итог + путь к артефакту + короткий вывод",
              tone: "accent"
            },
            consume: {
              title: "Следующий шаг",
              text: "Продолжение без лишнего шума",
              tone: "soft"
            },
            mainTitle: "Главное окно",
            workerTitle: "Окно сабагента",
            artifactTitle: "Артефакт handoff",
            mainLoadPercents: [34, 36, 36, 44],
            workerLoadPercents: [12, 74, 61, 0],
            mainWindowLines: [
              {
                text: "Цель: подтвердить причину падения и подготовить безопасную правку."
              },
              {
                text: "Подзадача: проверь cache invalidation после rename и верни только вывод.",
                delegated: true
              },
              {
                text: "В окне остаются цель, ограничения и последние подтверждённые наблюдения."
              }
            ],
            workerWindowLines: [
              {
                text: "Получает только подзадачу, write scope и критерий готовности."
              },
              {
                text: "Читает релевантные файлы, ищет вызовы, запускает тесты и собирает длинный след поиска.",
                trace: true
              },
              {
                text: "Фиксирует спорные места и найденные зависимости отдельно от основного окна.",
                trace: true
              },
              {
                text: "Перед возвратом превращает находки в короткий итог и список артефактов.",
                result: true
              }
            ],
            artifactPendingLine: "Артефакт ещё не собран: длинная рабочая траектория остаётся внутри окна сабагента.",
            artifactLines: [
              "notes/subagent-cache.md с кратким итогом и спорными местами",
              "3 релевантных файла и участок diff вокруг invalidation",
              "Вывод проверки: баг воспроизводится после rename + stale key"
            ],
            returnedLine:
              "Возврат: короткий вывод + путь к артефакту + 3 файла для следующего шага.",
            continueLine:
              "Следующий шаг: главный агент читает артефакт точечно и продолжает без полного trail сабагента.",
            note:
        ...
        },
        {
          ...
          viz: {
            type: "approval-flow",
            title: "Как работает маршрутизация прав",
            caption:
              "Один и тот же агент не должен иметь одинаковые права на каждом шаге. Хороший harness меняет доступ вместе с фазой работы и выносит опасные действия на явное подтверждение.",
            stages: ["Разбор", "Подготовка", "Запрос", "Выполнение"],
            stageDurations: [2800, 3200, 3400, 3400],
            statuses: {
              analyze:
                "На этапе разбора агент видит безопасный набор действий: читать, искать, смотреть структуру проекта и собирать признаки проблемы.",
              prepare:
                "Когда гипотеза созрела, harness открывает ограниченную правку в нужной зоне и оставляет опасные действия за пределами автоматического контура.",
              request:
                "Если нужен сетевой вызов, запись вне scope или потенциально опасная команда, действие превращается в явный запрос на подтверждение.",
              execute:
                "После подтверждения разрешается только конкретный шаг, а не бессрочное расширение прав на всю оставшуюся сессию."
            },
            phaseTitle: "Текущая фаза",
            phaseCards: [
              {
                title: "Разбор",
                text: "Чтение файлов, поиск, просмотр логов и проверка гипотез без побочных эффектов.",
                tone: "soft"
              },
              {
                title: "Подготовка правки",
                text: "Локальная запись в ограниченном scope и безопасные проверки внутри песочницы.",
                tone: "accent"
              },
              {
                title: "Запрос подтверждения",
                text: "Опасное действие выносится из общего цикла в явный gate с описанием риска.",
                tone: "warning"
              },
              {
                title: "Выполнение",
                text: "Разрешён только конкретный подтверждённый шаг, после чего система возвращается к обычным ограничениям.",
                tone: "accent"
              }
            ],
            policyTitle: "Набор действий",
            policyItems: [
              {
                label: "Чтение и поиск",
                stateByStage: ["allowed", "allowed", "allowed", "allowed"]
              },
              {
                label: "Локальная правка в scope",
                stateByStage: ["blocked", "allowed", "allowed", "allowed"]
              },
              {
                label: "Сетевой вызов / запись вне scope",
                stateByStage: ["blocked", "blocked", "pending", "approved"]
              },
              {
                label: "Опасная shell-команда",
                stateByStage: ["blocked", "blocked", "pending", "approved"]
              }
            ],
            sandboxTitle: "Песочница и запрос",
            sandboxLines: [
              "Sandbox: read/search доступны без подтверждения.",
              "Scoped write: правка разрешена только в затронутой зоне проекта.",
              "Approval request: `npm publish` или внешний HTTP вызов требуют отдельного approve.",
              "После шага расширенные права не остаются висеть на всей сессии."
            ],
            requestTitle: "Запрос на подтверждение",
            requestPending:
              "Пока опасного шага нет, система не отвлекает пользователя лишними запросами.",
            requestLine:
              "Нужен `git push` во внешний remote: покажи цель, риск и разовый scope разрешения.",
            resultLine:
              "Шаг выполнен по явному approve; дальше агент снова работает в обычном режиме доступа."
          },
              ...
              children: [
                ...
                },
                {
                  ...
                  viz: {
                    ...
                    title: "Как работает checkpoint",
                    caption:
                      "Checkpoint нужен не для архивирования ради архивирования, а чтобы переживать рискованные ветки работы без повторной сборки состояния из длинной истории.",
                    stages: ["Стабильная точка", "Рискованный ход", "Откат", "Продолжение"],
                    stageDurations: [2800, 3400, 3200, 3400],
                    statuses: {
                      stable:
                        "Агент фиксирует рабочее состояние: подтверждённую гипотезу, текущий набор файлов и последнюю стабильную проверку.",
                      risky:
                        "Дальше можно пробовать рискованный путь: серия правок, новые команды, перестройка конфигурации или спорный рефакторинг.",
                      restore:
                        "Если гипотеза ломается, checkpoint возвращает проект и рабочий контекст к последнему устойчивому снимку.",
                      continue:
                        "После отката работа продолжается не с нуля, а с восстановленного состояния и короткого вывода о том, что уже признано тупиковой веткой."
                    },
                    workspaceTitle: "Рабочее состояние",
                    workspaceLines: [
                      {
                        text: "Базовая ветка: причина сбоя локализована, тест воспроизводит ошибку."
                      },
                      {
                        text: "Рискованный ход: серия правок в rename-flow и новый прогон интеграционных тестов.",
                        risky: true
                      },
                      {
                        text: "Откат: возвращаем файлы и рабочий контекст к последнему стабильному снимку.",
                        restore: true
                      },
                      {
                        text: "Продолжение: сохраняем знание, что rename-flow не решает проблему, идём дальше",
                        continued: true
                      }
                    ],
                    checkpointTitle: "Checkpoint",
                    checkpointPending:
                      "Снимок ещё не создан: если сейчас уйти в рискованный путь, придётся вручную восстанавливать состояние.",
                    checkpointLines: [
                      "Снимок файлов перед экспериментом",
                      "Последний стабильный результат проверки",
                      "Краткая заметка: что уже подтверждено и что ещё не проверено"
                    ],
                    loadTitle: "Стоимость восстановления",
                    loadPercents: [18, 82, 26, 34],
                    loadLabels: [
                      "Есть безопасная точка возврата",
                      "Без checkpoint откат стоил бы дорого",
                      "Снимок быстро восстанавливает рабочее состояние",
                      "Новая ветка продолжается без пересборки всего контекста"
                    ],
                    restoredLine:
                      "Восстановлено: стабильные файлы + последняя рабочая проверка + заметка о неудачном эксперименте."
                  },
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          viz: {
            type: "tool-loading",
            title: "Как работает отложенная загрузка",
            caption:
              "Не нужно грузить в активное окно все определения инструментов сразу. Сначала выбирается узкий набор под задачу, и только он получает место в prompt.",
            stages: ["Каталог", "Выбор", "Загрузка", "Рабочее окно"],
            stageDurations: [2500, 3200, 3200, 3400],
            statuses: {
              catalog:
                "У системы может быть большой каталог MCP-серверов и инструментов, но это ещё не значит, что все их схемы должны лежать в prompt.",
              select:
                "Сначала harness смотрит на задачу и выбирает только релевантный поднабор инструментов для текущего шага.",
              load:
                "Лишь после этого в активное окно попадают описания выбранных tools и их схема ответа.",
              work:
                "Дальше агент работает уже с компактным набором инструментов, а не с каталогом из десятков интеграций."
            },
            ...
            selectedTitle: "Выбраны для задачи",
            selectedLines: [
              "filesystem.search",
              "filesystem.read",
              "git.diff"
            ...
            skippedLine:
              "Остальные tools остаются вне prompt",
            windowTitle: "Активное окно",
            windowLines: [
              "Загружены только 3 схемы инструментов",
              "Токены не тратятся на tools без необходимости",
              "Если фаза меняется, набор можно пересобрать"
            ],
